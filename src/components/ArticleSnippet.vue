    ...
    <div class="flex p-6 bg-white border-2 border-primary rounded-lg transform -translate-x-1 -translate-y-1">
      ...
      <div class="w-11/12">
        ...
        <div class="text-primary font-semibold -ml-2">
          <g-link class="uppercase text-xs hover:bg-primary hover:text-white py-1 px-2 rounded-full transition duration-100 ease-in-out" v-for="tag in article.tags" :key="tag.id" :to="tag.path">{{ tag.title }}</g-link>
        </div>
      </div>
    </div>
  </g-link>
...
/* .article:hover::after {
  ...
  left: 0px;
  bottom: 0px;
  box-shadow: rgba(4, 25, 50, 0.7) 3px 0px 22px;
...
.article::after {
  ...
  background-color: rgb(4, 25, 50, 0);
  position: absolute;
  left: 0px;
  bottom: 0px;
  margin: 0px 0px 0px -1px;
...
.dark .article:hover::after {
  ...
  left: 0px;
  bottom: 0px;
  box-shadow: rgba(208, 217, 226, 0.7) 3px 0px 22px;
  margin: 0px 0px 0px -1px;
  box-sizing: content-box;
}
...
.dark .article::after {
  ...
  background-color: rgb(208, 217, 226, 0);
  position: absolute;
  left: 0px;
  bottom: 0px;
  margin: 0px 0px 0px -1px;

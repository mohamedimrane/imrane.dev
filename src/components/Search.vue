    ...
    <div class="relative">
      ...
      <div v-if="searchResults.length > 0" class="absolute left-0 right-0 z-10 mt-2 mb-4 overflow-hidden overflow-y-auto text-left normal-case bg-white border rounded shadow-sm" style="max-height: 32rem">
        <div class="flex flex-col">
          <g-link v-for="result in searchResults" :key="result.id" :to="result.path" class="p-4 font-semibold border-b cursor-pointer dark:bg-darkmode-dark-teal dark:text-white border-light-text10 hover:bg-light-text10">
            {{ result.title }}
            <span class="block my-1 text-sm font-normal text-blue40">{{ result.node.summary }}</span>
          </g-link>
        </div>
      </div>
      <div v-if="searchResults.length === 0 & searchTerm.length > 2" class="w-full p-4 mt-1 font-normal border-b cursor-pointer bg-light-text10">
        <p class="my-0">

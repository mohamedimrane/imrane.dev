<template>
  <ArticleLayout>
    <main class="mx-auto mt-16 lg:mt-32 container-inner">
      ...
      <ul class="mt-2">
        <g-link tag="li" class="inline-block px-3 py-1 mr-3 text-sm bg-gray-400 bg-opacity-25 rounded cursor-pointer dark:text-white hover:bg-opacity-40"
          v-for="tag in $page.article.tags"
          :key="tag.id" :to="tag.path"
        >{{ stringCapitalize(tag.title) }}</g-link>
      </ul>
